...
export class HeaderBarComponent implements OnInit {
  ...
  getCurrentRouteIndicator(indicatorName: string, primaryField: string) {
    return this.router.isActive(
      `/indicator/${indicatorName}/${primaryField}`,
      true
    );
  }
  ...
  goToView(indicator: any) {
    if (indicator === "logo" || indicator === "home") {
      this.router.navigate([
        `dashboard/${this.currentUser.roles[0].description.toLowerCase()}`,
      ]);
      return;
    }
    ...
    if (
      ...
      this.indicatorService.getIndicatorsByUser(this.currentUser.id).subscribe(
        (res) => {
          ...
          this.indicators = res.data.filter(
            (indicator) =>
              (indicator.indicator.type =
                indicator.indicator.name.toLocaleLowerCase())
          );
