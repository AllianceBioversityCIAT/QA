...
export class DetailIndicatorComponent implements OnInit {
  ...
  getEvaluationStatus() {
    this.evaluationService
      .getEvaluationStatus(this.params.indicatorId)
      .subscribe((resp) => {
  ...
  getDetailedData() {
    this.evaluationService
      .getDataEvaluation(this.currentUser.id, this.activeRoute.snapshot.params)
      .subscribe(
        (res) => {
  ...
  addGeneralComment(data) {
    if (this.formData.general_comment.invalid) {
      this.alertService.error(
        "Reply to general comment can not be empty",
        false
      );
      return;
    }
  ...
  validateCommentAvility(field, is_embed) {
    ...
    switch (userRole) {
      ...
      case Role.crp:
        avility =
          field.enable_crp == 1 ? field.enable_comments : field.enable_crp;
