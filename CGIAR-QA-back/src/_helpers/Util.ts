...
class Util {
  ...
  static login = async (username: string, password: string) => {
    try {
      ...
      let user: any;
      let marloUser = await userRepository.findOne({
        where: [
      ...
      if (marloUser) {
        let isMarlo = await AuthController.validateAD(marloUser, password);
        if (isMarlo) {
          user = marloUser;
        } else {
          throw new BaseError(
      ...
      const [generalConfig, currentCycle] = await Promise.all([
        grnlConfg.find({
          where: {
            roleId: In(user.roles.map((role) => role.id)),
  ...
  static parseEvaluationsData(rawData, type?) {
    ...
    for (let index = 0; index < rawData.length; index++) {
      if (!type) {
        ...
        !rawData[index]["meta_is_primay"] &&
        rawData[index]["meta_include_detail"]
      ) {
        response.push(this.formatResponse(rawData[index], type));
      }
    }
  ...
  static parseCommentData(rawData, type?) {
    ...
    for (const key in grouped_data) {
      ...
      if (grouped_data.hasOwnProperty(key)) {
        let itm = [
          {
            indicator_view_name: key,
            label: 0,
            value: 0,
            type: "warning",
            status: "answered",
            enable_crp: element[0].enable_crp,
            enable_assessor: element[0].enable_assessor,
            primary_field: element[0].primary_field,
      ...
      data_set: [
        {
          data: [],
          label: "CRP - Approved",
          sql_name: "approved_comment_crp",
          backgroundColor: "#ffca30",
        },
        {
          data: [],
          label: "CRP - Rejected",
          sql_name: "rejected_comment_crp",
          backgroundColor: "#F1B7B7",
        },
        {
          data: [],
          label: "CRP - No responded",
          sql_name: "crp_no_commented",
          backgroundColor: "#0f8981",
        },
        {
          data: [],
          label: "Assessor - Commented",
          sql_name: "assessor_comments",
          backgroundColor: "#61b33e",
        },
        {
          data: [],
          label: "Assessor - Approved without comment",
          sql_name: "approved_no_comment",
          backgroundColor: "#2e7636",
        },
        {
          data: [],
          label: "Total",
          sql_name: "comments_total",
          backgroundColor: "#b73428",
        },
      ],
    };
  ...
  static createOrReturnUser = async (authToken: any): Promise<any> => {
    ...
    try {
      ...
      let crp = await crpRepository.findOneOrFail({
        where: { crp_id: authToken.crp_id },
      });
      let crpRole = await roleRepository.findOneOrFail({
        where: { description: RolesHandler.crp },
      });
  ...
  static createMetaForIndicator = async (
    indicator: QAIndicators,
    primary_field: string
  ) => {
    let pols_meta = getConnection()
  ...
  static createEvaluations = async (
    indiByUsr: QAIndicatorUser,
    indicator: QAIndicators
  ): Promise<any> => {
    const evaluationsRepository = getRepository(QAEvaluations);
    try {
      let evaluations = await evaluationsRepository.find({
        where: { indicator_user: indiByUsr.id },
      });
      let response;
      if (evaluations.length > 0) {
        return [];
      } else {
        let view_data = await createQueryBuilder(indicator.view_name).getMany();
        let savePromises = [];
  ...
  static createCommentsExcel = async (
      ...
      for (let i = 0; i < rows.length; i++) {
        if (rows[i]) {
          ...
          let row = {
            ...
            field: rows[i].display_name
              ? rows[i].display_name
              : "General Comment",
            value: field_value[0].col_name || "",
            crp_approved:
              rows[i].crp_approved != null
                ? rows[i].crp_approved == 1
                  ? "Yes"
                  : "No"
                : "",
            reply: rows[i].reply,
  ...
  static createRawCommentsExcel = async (
    headers: Partial<excel.Column>[],
    rows: any[],
    sheetName: string
  ): Promise<Buffer> => {
  ...
  static createComment = async (
    ...
    try {
      ...
      let evaluation = await evaluationsRepository.findOneOrFail({
  ...
  static createTag = async (userId, tagTypeId, commentId) => {
    ...
    try {
      let user = await userRepository.findOneOrFail({ where: { id: userId } });
      let tagType = await tagTypeRepository.findOneOrFail({
        where: { id: tagTypeId },
      });
  ...
  static calculateStatusMIS = (evaluation, batches) => {
    ...
    try {
      if (evaluation.phase_year == "2021") {
        let current_batch = batches.find((b) =>
            ...
            if (
              ...
              if (evaluation.pending_replies) {
                ...
                status = evaluation.require_second_assessment
                  ? "in_progress"
                  : StatusHandlerMIS[evaluation.status];
              }
            }
            break;
