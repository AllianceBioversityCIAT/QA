...
class IndicatorsController {
  ...
  static getCRP = async (req: Request, res: Response) => {
    ...
    try {
      ...
      res
        .status(200)
        .json({ data: crp, message: `CRP ${crp.id} - ${crp.acronym} loaded.` });
    } catch (error) {
      res.status(404).json({ message: "Could not access to indicators." });
    }
  };
  ...
  static getIndicatorsByUser = async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (isAdmin) {
        ...
        const indicators = await queryRunner.connection.query(
          query,
          parameters
        );
  ...
  static createIndicator = async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (indicator.primary_field && indicator.primary_field !== " ") {
        let indicatorMeta = await Util.createMetaForIndicator(
          indicator,
  ...
  static assignIndicatorToUser = async (req: Request, res: Response) => {
    ...
    if (crpId && indicator_id === null) {
      ...
      try {
        ...
        res
          .status(200)
          .json({ message: "Indicator by user saved", data: res_ });
      } catch (error) {}
    } else {
      try {
        selectedUser = await userRepository.findOneOrFail(user_id);
        selectedIndicator = await indicatorRepository.findOneOrFail(
          indicator_id
        );
        hasAssignedIndicators = await indicatorbyUsrRepository
      ...
      try {
        ...
        res
          .status(200)
          .json({ message: "Indicator by user saved", data: userbyIndicator });
      } catch (e) {
  ...
  static getItemStatusByIndicator = async (req: Request, res: Response) => {
    ...
    try {
      ...
      const [query, parameters] =
        await qrMetas.connection.driver.escapeQueryWithParameters(
          queryMetas,
          { indicator },
          {}
        );
      ...
      for (const meta of allMetas) {
        ...
        try {
          const [query, parameters] =
            await queryRunnerNotApplicable.connection.driver.escapeQueryWithParameters(
              totalEvaluationsByIndicator[meta.view_name][meta.display_name][
                "queryNotApplicable"
              ],
              {},
          ...
          let notApplicableCount =
            await queryRunnerNotApplicable.connection.query(query, parameters);
          totalEvaluationsByIndicator[meta.view_name][meta.display_name][
            "notApplicable"
      ...
      for (let i = 0; i < allItems.length; i++) {
        switch (allItems[i].approved_no_comment) {
          ...
          case null:
            ...
            totalEvaluationsByIndicator[allItems[i].indicator_view_name][
              allItems[i].display_name
            ]["pending"] -= allItems[i].assessment_with_comments;
            break;
          default:
            break;
        }
  ...
  static getItemListStatusMIS = async (req: Request, res: Response) => {
    ...
    try {
      const indicator_view_name = await indicatorRepository.find({
        where: { id: id },
        select: ["view_name"],
      });
      ...
      const batches = await batchesRepository.find({
        where: { phase_year: AR },
      });
      ...
      res
        .status(404)
        .json({ message: "Items for MIS cannot be retrieved", data: error });
    }
  };
