            ...
            if (isAdmin) {
                ...
                const [query, parameters] = await queryRunner.connection.driver.escapeQueryWithParameters(
                    `
                        ...
                        ( SELECT COUNT(DISTINCT id) FROM qa_comments WHERE metaId = meta.id  AND evaluationId = evaluations.id  AND is_visible = 1 AND is_deleted = 0 AND approved_no_comment IS NULL AND tpb = 1 ) AS tpb_count,
