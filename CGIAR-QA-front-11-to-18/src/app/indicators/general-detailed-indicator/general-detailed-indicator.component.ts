...
export class GeneralDetailedIndicatorComponent implements OnInit {
  ...
  tooltips = {
    ...
    all_approved:
      "Setting this option true, will approved all items without comments.",
  ...
  constructor(
    ...
    this.activeRoute.params.subscribe((routeParams) => {
      ...
      this.titleService.setTitle(
        `${this.currentType} / QA-${this.params.type
          .charAt(0)
          .toUpperCase()}${this.params.type.charAt(1).toUpperCase()}-${
          this.params.indicatorId
        }`
      );
  ...
  validateComments() {
    ...
    for (let index = 0; index < this.detailedData.length; index++) {
      ...
      response =
        parseInt(element.replies_count) > 0 ||
        this.formTickData.controls[index].value.isChecked;
  ...
  validateUpdateEvaluation() {
    let checked_row = this.detailedData
      .filter((data, i) =>
        this.formTickData.controls[i].value.isChecked ? data : undefined
      )
      .map((d) => d.field_id);
    let commented_row = this.detailedData
      .filter((data) => data.replies_count != "0")
      .map((d) => d.field_id);
    let availableData = this.detailedData.filter(
      (data) => data.enable_comments
    );
    if (
  ...
  onChangeSelectAll(e) {
    ...
    if (e) {
      this.formTickData.controls.map((value, i) =>
        this.detailedData[i].replies_count == "0"
          ? value.get("isChecked").setValue(true)
          : value.get("isChecked")
      );
      selected_meta = this.detailedData
        .filter((data, i) =>
          this.formTickData.controls[i].value.isChecked ? data : undefined
        )
        .map((d) => d.field_id);
      ...
      this.formTickData.controls.map((value) =>
        value.get("isChecked").setValue(false)
      );
      selected_meta = this.detailedData
        .filter((data, i) =>
          !this.formTickData.controls[i].value.isChecked ? data : undefined
  ...
  checkAllIsApproved() {
    ...
    this.formTickData.controls.forEach((value, i) => {
      statusByField.push({
        ...
        value:
          this.detailedData[i].replies_count != "0" ||
          value.get("isChecked").value ||
          this.detailedData[i].enable_comments == false
            ? true
            : false,
      });
    });
    this.approveAllitems = statusByField.find((e) => e.value == false)
      ? false
      : true;
  ...
  getDetailedData() {
    this.evaluationService
      .getDataEvaluation(this.currentUser.id, this.params)
      .subscribe(
        (res) => {
          ...
          this.gnralInfo = {
            ...
            status_update: null,
            general_comment_updatedAt:
              this.detailedData[0].general_comment_updatedAt,
            general_comment_user: this.detailedData[0].general_comment_user,
            requires_second_assessment:
              this.detailedData[0].require_second_assessment,
          ...
          this.activeCommentArr = Array<boolean>(this.detailedData.length).fill(
            false
          );
          this.evaluationService
            .getAssessorsByEvaluation(this.gnralInfo.evaluation_id)
            .subscribe(
              (res) => {
                this.assessed_by_r1 = res.data.assessed_r1;
                this.assessed_by_r2 = res.data.assessed_r2;
                if (this.assessed_by_r1) {
                  this.currentUserHasAssessed =
                    this.assessed_by_r1.indexOf(this.currentUser.username) >= 0
                      ? true
                      : false;
                }
              },
  ...
  getCommentsExcel(evaluation) {
    ...
    let filename = `QA-${this.params.type
      .charAt(0)
      .toUpperCase()}${this.params.type.charAt(1).toUpperCase()}-${
      this.detailedData[0].result_code
  ...
  updateEvaluation(type: string, data: any) {
    ...
    this.evaluationService
      .updateDataEvaluation(evaluationData, evaluationData.evaluation_id)
      .subscribe(
        (res) => {
  ...
  markForSecondAssessment() {
    ...
    this.evaluationService
      ...
      .subscribe(
        (res) => {
          this.hideSpinner("spinner1");
          this.gnralInfo.requires_second_assessment =
            !this.gnralInfo.requires_second_assessment;
        },
        (error) => {
          this.hideSpinner("spinner1");
    ...
    switch (userRole) {
      ...
      case Role.asesor:
        if (this.gnralInfo.status === this.statusHandler.Pending) {
          avility = field.enable_assessor
            ? field.enable_comments
            : field.enable_assessor;
        } else {
          avility = field.enable_assessor
            ? field.enable_comments
            : field.enable_assessor;
  ...
  verifyIsLeadAssessor() {
    return this.currentUser.indicators.find(
      (el) =>
        el.indicator.view_name.split("qa_")[1] == this.indicatorType &&
        el.isLeader
    )
      ? true
      : false;
  ...
  getCurrentTypeUrl(): string {
    const baseUrl =
      this.params.type === "innovation_use_ipsr" ? "ipsr/" : "result/";

    const outlet =
      this.params.type === "innovation_use_ipsr" ? "detail/" : "result-detail/";
    ...
    const url =
      ...
      this.detailedData[0].result_code +
      "/general-information?" +
      `phase=${this.detailedData[0].version}`;
