SELECT
        ...
        (
            SELECT
                GROUP_CONCAT(
                    ...
                    '<br>',
                    IF(
                        (rie.email IS NOT NULL),
                        CONCAT('<b>Email: </b>', rie.email, '<br>'),
                        ''
                    ),
                    (
                        SELECT
                            CONCAT(
                                ...
                                GROUP_CONCAT(
                                    ' -',
                                    e2.name SEPARATOR '<br>'
                                )
                            )
                        FROM
                            prdb.result_ip_expertises rie2
                            JOIN prdb.expertises e2 ON e2.expertises_id = rie2.expertises_id
                        WHERE
            ...
            FROM
                prdb.result_ip_expert rie
                JOIN prdb.clarisa_institutions cin4 ON cin4.id = rie.organization_id
            WHERE
                rie.result_id = r.id
                AND rie.is_active = 1
        ),
        'Data not provided.'
    ) AS innovation_pack_experts,
    IF(
        (rip.experts_is_diverse = 1),
        'Yes, the group of experts is diverse',
        CONCAT(
            'No, the list of experts is not yet as diverse as desired and can be improved by adding the following expert groups',
            '<br>',
            rip.is_not_diverse_justification
        )
    ) AS experts_is_diverse,
    IFNULL(
        CONCAT(
            '<b>Consensus/commitment among the Initiative and Work Package leadership and partners on the proposed Innovation, country and targets: </b><br>',
            (
                SELECT
                    ciwp.name
                FROM
                    prdb.consensus_initiative_work_package ciwp
                WHERE
                    ciwp.consensus_initiative_work_package_id = rip.consensus_initiative_work_package_id
            ),
            '<br><br>',
            '<b>CGIAR country convener is aware of the proposed innovation package and scaling readiness exercise: </b><br>',
            (
                SELECT
                    rc.name
                FROM
                    prdb.relevant_country rc
                WHERE
                    rc.relevant_country_id = rip.relevant_country_id
            ),
            '<br><br>',
            '<b>CGIAR Regional Leadership is aware of the proposed innovation package and scaling readiness exercise: </b><br>',
            (
                SELECT
                    rl.name
                FROM
                    prdb.regional_leadership rl
                WHERE
                    rl.regional_leadership_id = rip.regional_leadership_id
            ),
            '<br><br>',
            '<b>CGIAR Regional Integrated Initiative leadership is aware of the proposed innovation package and scaling readiness exercise: </b><br>',
            (
                SELECT
                    ri.name
                FROM
                    prdb.regional_integrated ri
                WHERE
                    ri.regional_integrated_id = rip.regional_integrated_id
            ),
            '<br><br>',
            '<b>Interested in receiving active support to ensure (gender-)responsible scaling?: </b><br>',
            (
                SELECT
                    ab.name
                FROM
                    prdb.active_backstopping ab
                WHERE
                    ab.active_backstopping_id = rip.active_backstopping_id
            )
        ),
        'Data not provided.'
    ) AS consensus_and_consultation,
    IFNULL (
        (
            SELECT
                GROUP_CONCAT(
                    ...
                    IFNULL(
                        (
                            ...
                            WHERE
                                e3.result_id = r3.id
                                AND e3.is_active = 1
                                AND e3.evidence_type_id = 4
                        ),
                        '<b>Reference materials: </b> Not provided.<br>'
                    ) SEPARATOR '<br><br>'
                )
            FROM
    ...
    IFNULL(
        (
            IF(
                ...
                'Yes, an expert workshop was organized',
                'No expert workshop was organized'
            )
        ),
        'Data not provided.'
    ) AS expert_workshop_organized,
    IF (
        rip.is_expert_workshop_organized = 1,
        IFNULL(
            (
                SELECT
                    GROUP_CONCAT(
                        ...
                        '<br>',
                        '<b> Workshop role: </b>',
                        riewo.workshop_role SEPARATOR '<br><br>'
                    )
                FROM
                    prdb.result_ip_expert_workshop_organized riewo
                WHERE
                    riewo.result_id = r.id
                    AND riewo.is_active = 1
            ),
            '<Not applicable>'
        ),
        '<Not applicable>'
    ) AS workshop_facilitators,
    IF (
        rip.is_expert_workshop_organized = 1,
        IFNULL(
            (
            ...
            SELECT
                CONCAT(
                    (
                        ...
                        FROM
                            prdb.clarisa_initiatives ci6
                        WHERE
                            rbi.inititiative_id = ci6.id
                    ),
                    '<br>',
                    IF(
                        (riib.is_determined = 1),
                        '<b>This is yet to be determinated.</b>',
                        CONCAT(
                            '<b>Current calendar year (USD value): </b>',
                            riib.current_year,
                            '<br>',
                            '<b>Next calendar year (USD value): </b>',
                            riib.next_year
                        )
                    )
                )
            FROM
                prdb.result_initiative_budget riib
            WHERE
                riib.result_initiative_id = rbi.id
        ),
        'Data not provided.'
    ) AS expected_lead_initiative_investment,
    IFNULL(
        (
            SELECT
                GROUP_CONCAT(
            ...
            FROM
                prdb.result_initiative_budget riib2
                JOIN prdb.results_by_inititiative rbi5 ON rbi5.id = riib2.result_initiative_id
                AND rbi5.initiative_role_id = 2
                JOIN prdb.clarisa_initiatives ci5 ON ci5.id = rbi5.inititiative_id
            WHERE
                rbi5.result_id = r.id
                AND riib2.is_active = 1
                AND rbi5.is_active = 1
        ),
        '<Not applicable>'
    ) AS expected_contributing_initiative_investment,
    IFNULL(
        (
            SELECT
                GROUP_CONCAT(
                    ...
                    IF(
                        ...
                        '<b>This is yet to be determinated.</b>',
                        CONCAT(
                            '<b>Current calendar year (in-kind + in-cash USD value): </b>',
                            nppb.in_kind,
                            '<br>',
                            '<b>Next calendar year (in-kind + in-cash USD value): </b>',
                            nppb.in_cash
                        )
                    ) SEPARATOR '<br><br>'
                )
            FROM
                ...
                JOIN prdb.non_pooled_project npp2 ON npp2.id = nppb.non_pooled_projetct_id
                AND npp2.is_active = 1
                AND npp2.results_id = r.id
            WHERE
                nppb.is_active = 1
        ),
        'Data not provided.'
    ...
    IFNULL(
        (
            ...
            FROM
                prdb.result_institutions_budget rinsb
                JOIN prdb.results_by_institution rbin3 ON rbin3.id = rinsb.result_institution_id
                AND rbin3.result_id = r.id
                AND rbin3.is_active = 1
                JOIN prdb.clarisa_institutions ci6 ON ci6.id = rbin3.institutions_id
            WHERE
                rinsb.is_active = 1
        ),
        'Data not provided.'
    ) AS expected_partner_investment,
    IFNULL(
        (
            SELECT
                GROUP_CONCAT(
